<!--
původní kód Tomáš Rieger
opravy Jiří Vavrejn
opravy Petr Sládek 
-->
<?php

/* PS: Následující řádka způsobuje načtení textu z webové stránky
* ze vstupního pole do proměnné. Pokud máte server nastavený podobně
* jako já (tak jak se doporučuje z bezpečnostního hlediska), budete
* to potřebovat. Na serveru KIVu to ale není nutné.
* Pokud s tím nesouhlasíte, následující řádku zakomentujte. */
$text = $_POST["text"];
...
/* PS: Výpis čísla řádky. */
    ...
    switch($n)	
    {
      ...
      case 32	: echo 'prilis velke zanoreni';
        break;
	  case 33   : echo 'ocekavano "while"'; /* MB: nova chybova hlaska */
        break;
    }
/* PS: Výpis čísla řádky. */
    echo "\ncislo radky: $lineno\n";
...
/* PS: Tato funkce nepracuje s globální proměnnou $pom. */
/*	global $pom;*/
...
function test($s1,$s2,$n) {
	...
	global $sym;
/* PS: Jedná se IMHO o bug, protože $pom nemá pracovat s globální proměnnou. */
/*	global $pom;*/
...
/* PS: Je velmi divné, že implementace PL/0 ve všech jazycích registrují
		'do' u 'if', i když je funkční pouze 'then'. */
            ...
            else
            {
                ...
                else
                {
                    if ($sym == Kwhilesym) { /*zacina cyklus while*/
					 ...
					 else if ($sym == Kdosym) {           /* MB: zde zacina implementace cyklu do-while */
						                                  /* MB: nejprve se zpracovava telo "do" */
                        $loop_beginning = $cx;            /* MB: aktualni pozice - zacatek cyklu - kam se bude skakat */
                        getsym();                         /* MB: dalsi symbol */
                        $pom = nuluj();                   /* MB: vynulovani pomocne promenne */
                        sjednot($pom, $fsys);             /* MB: sjednoceni promennych $pom s $fsys */
                        $pom[Ksemicolon] = 1;             /* MB: statement konci strednikem */
                        statement($pom,$tx,$lev);         /* MB: rekurzivne generovat statement */
                        if ($sym == Ksemicolon) getsym(); /* MB: skoncil statement opravdu Ksemicolon? */
                        else error(5);                    /* MB: chyba */
                        if ($sym == Kwhilesym) getsym();  /* MB: nyni musi prijit "while" */
                        else error(33);                   /* MB: nespravny syntax */
						
                                                          /* MB: zde se zpracovava obsah podminky uvnitr "while" */
                        $pom = nuluj();                   /* MB: vynulovani pomocne promenne */
                        sjednot($pom, $fsys);             /* MB: sjednoceni promennych $pom s $fsys */
                        $pom[Ksemicolon] = 1;             /* MB: condition take konci strednikem */
                        condition($pom,$tx,$lev);         /* MB: vygenerovani celeho bloku condition */
                        $cx_jpc = $cx;                    /* MB: pozice instrukce kam se vygeneruje jpc - budeme ji modifikovat */
                        gen(Kjpc,0,0);                    /* MB: podminka neni splnena - preskakujeme dalsi instrukci a cyklus konci */
                        gen(Kjmp,0,$loop_beginning);      /* MB: podminka je splnena (resp. tato instr. neni preskocena), skaceme ulozeny na zacatek cyklu */
                        $code[$cx_jpc]->a = $cx;          /* MB: nyni nastavime cil skoku jpc */
                     }                                    /* MB: Konec implementace cyklu do-while */
                 }
              }
...
<head>
<title>Překladač PL/0 v PHP</title>
</head>
<body>
<link href="styl.css" type="text/css" rel="stylesheet">
<h1>Implementace překladače PL/0 v PHP</h1>
<b>Vypracoval Tomáš Rieger jako semestrální práci z předěmtu KIV/FJP vyučovaném na ZČU.</b><p>
...
else
{
	...
	$wsym[10] = Kwhilesym;
        ...
        /* PS: oprava komentáře, který patrně prošel automatickou náhradou textu */
   ...
   $declbegsys[Kconstsym] = $declbegsys[Kvarsym] = $declbegsys[Kprocsym] = 1;
    ...
    $statbegsys[Kbeginsym] = $statbegsys[Kcallsym] = $statbegsys[Kifsym] = $statbegsys[Kwhilesym] = $statbegsys[Kdosym] = 1; /* MB: přidán symbol "do" do množiny počátečních symbolů */
...
Stisknutím tlačítka Debugger se otevře nové okno s online debuggerem PL/0.<p>
